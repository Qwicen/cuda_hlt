    ...
    GIVEN( "Grid of hits with the same z coordinate at all stations" ) {
        ...
        WHEN( "Track equidistant from 4 hits and parallel to the axis OZ (x=0.5, y=0.5, dx=0, dy=0, z=0)" ) {
            ...
            THEN( "The closest hit at all stations is ( 0, 0) or ( 0, 1) or ( 1, 0) or ( 1, 1)." 
                ...
                const std::vector<int> closest_hits1 = {8, 17, 26, 35};
                for (int i_station = 0; i_station < Muon::Constants::n_stations; i_station++) {
                    const int closest_idx = closest_hits[i_station];
                    CHECK(host_features[offset::DTS + i_station] == muon_hits_events[0].delta_time[closest_idx]);
                    CHECK(host_features[offset::TIMES + i_station] == muon_hits_events[0].time[closest_idx]);
                    CHECK(host_features[offset::CROSS + i_station] + muon_hits_events[0].uncrossed[closest_idx] == 2);
                    CHECK(
                        (host_features[offset::RES_X + i_station] == -sqrt((4 + 9 * i_station) / 3) || 
                        (host_features[offset::RES_X + i_station] == -sqrt((7 + 9 * i_station) / 3) ||
                        (host_features[offset::RES_X + i_station] == -sqrt((5 + 9 * i_station) / 3) ||
                        (host_features[offset::RES_X + i_station] == -sqrt((8 + 9 * i_station) / 3)
                    );
                    CHECK(
                        (host_features[offset::RES_Y + i_station] == -sqrt(2 * (4 + 9 * i_station) / 3) || 
                        (host_features[offset::RES_Y + i_station] == -sqrt(2 * (7 + 9 * i_station) / 3) ||
                        (host_features[offset::RES_Y + i_station] == -sqrt(2 * (5 + 9 * i_station) / 3) ||
                        (host_features[offset::RES_Y + i_station] == -sqrt(2 * (8 + 9 * i_station) / 3)
                    );
                }
            }
            cudaFree(dev_track);
        ...
        WHEN( "Track mathes hit and parallel to the axis OZ (x=0.5, y=0.5, dx=0, dy=0, z=0)" ) {

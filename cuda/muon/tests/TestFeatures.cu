...
SCENARIO( "Track inside grid of hits" ) {
    ...
    GIVEN( "Grid of hits with the same z coordinate at all stations" ) {
        ...
        WHEN( "Track parallel to the axis OZ (x=0.9, y=0.9, dx=0, dy=0, z=0)" ) {
            ...
            THEN( "The closest hit at all stations is ( 1, 1). Indices: 8, 17, 26, 35" ) {
                ...
                for (int i_station = 0; i_station < Muon::Constants::n_stations; i_station++) {
                    ...
                    CHECK(host_features[offset::RES_X + i_station] == muon_hits_events[0].RES_X[closest_idx]);
                    CHECK(host_features[offset::RES_Y + i_station] == muon_hits_events[0].RES_Y[closest_idx]);
                }
            }
            cudaFree(dev_track);

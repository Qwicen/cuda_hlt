...
bool reconstructMultiPVFromTracks(VeloState * tracks2use, Vertex * outvtxvec, int number_of_tracks,
  uint * number_of_vertex, int event_number, bool * tracks2disable, XYZPoint * seeds, uint * number_of_seeds) 
{
  ...
  while(continue_fitting) {
    ...
    getSeeds( rtracks, beamspot, number_of_tracks, seeds, number_of_seeds, event_number, tracks2disable);
...
int run_PatPV_on_CPU (
  ...
  for(int i_event = 0; i_event < number_of_events; i_event++) {
    ...
    // number of tracks in event i_event
    int number_of_tracks = host_number_of_tracks_pinned[i_event];

    //pointer to beginning of tracks of event i_event
    ...
    //recosntruc PVs for event i_event
    reconstructMultiPVFromTracks(kalman_states, outvtxvec, number_of_tracks, number_of_vertex, i_event, tracks2disable, seeds, number_of_seeds);
  }

...
parser.add_argument('-p', '--momentum', action='store_true', dest='p', default=False,
                    help='Use momentum instead of q/p (default false).')
parser.add_argument('-s', '--stddev', action='store_true', dest='stddev', default=False,
...
if args.p:
    firstRes = np.array([pResFit(inTree,'1./first_qop',bin,args.stddev) for bin in bins])
    bestRes = np.array([pResFit(inTree,'1./best_qop',bin,args.stddev) for bin in bins])
    yLabel = '#sigma_{#it{p}}/#it{p}'
else:
    firstRes = np.array([qopResFit(inTree,'first_qop',bin,args.stddev) for bin in bins])
    bestRes = np.array([qopResFit(inTree,'best_qop',bin,args.stddev) for bin in bins])

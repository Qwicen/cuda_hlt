...
def ipRes3D(tree, var, ptBin):
    ...
    tree.Draw(var+'>>h','ghost==0 && 1000./best_pt>={} && 1000./best_pt<{}'.format(ptBin[0],ptBin[1]),'goff')
...
def makeGraph(tree, var, resFunc):
    ...
    g = ROOT.TGraphErrors(len(centers), centers, array('d',1000.*res[:,0]), widths, array('d',1000.*res[:,1]))
...
if __name__ == '__main__':
    ...
    for graph in graphs: graph.Write()
    outFile.Write()
    outFile.Close()
